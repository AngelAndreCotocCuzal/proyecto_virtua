...
import ModoExamen from './ModoExamen'
...
export default function Dashboard({ token, onLogout }: any){
  ...
  const [diaFiltro, setDiaFiltro] = useState<string>(() => {
    ...
    return diasSemana[new Date().getDay()] ?? 'Todos'
  })
  ...
  const cursosFiltrados = diaFiltro === 'Todos'
    ? misCursos
    : misCursos.filter((curso) => normalizarTexto(curso.dia_semana || '') === normalizarTexto(diaFiltro))
          ...
          <div className="grid grid-cols-2 gap-6">
            ...
            <div className="col-span-2 bg-white p-4 rounded shadow">
              <div className="mb-3 flex flex-col gap-3">
                <h2 className="font-semibold">Mis Cursos Asignados</h2>
                <div className="flex flex-wrap gap-2">
                  {['Todos', 'Lunes', 'Martes', 'Miércoles', 'Jueves', 'Viernes', 'Sábado'].map((dia) => {
                    const activo = diaFiltro === dia
                    return (
                      <button
                        key={dia}
                        type="button"
                        onClick={() => setDiaFiltro(dia)}
                        className={`rounded-full px-3 py-1 text-xs font-medium transition ${activo ? 'bg-slate-800 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
                      >
                        {dia}
                      </button>
                    )
              ...
              <div className="space-y-2 max-h-64 overflow-y-auto pr-2 [&::-webkit-scrollbar]:w-2 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-slate-300/70 hover:[&::-webkit-scrollbar-thumb]:bg-slate-400/80">
                {cursosFiltrados.length === 0 ? (
                  <p className="text-sm text-slate-400">No hay cursos para el día seleccionado.</p>
                ...
                {pcs.length === 0 ? <p className="text-sm text-slate-400">No hay agentes reportando tráfico...</p> : 
                  pcs.map(p => (
                    <div key={p.id || p.ip} className="flex justify-between items-center border-b pb-2">
                      <div>
                        <div className="font-medium text-slate-700">{p.alumno ? p.alumno.nombre : 'PC en Sala (Esperando Login)'}</div>
